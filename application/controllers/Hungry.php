...
class Hungry extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['getcategory'] =  $this->hungry_model->getcategory();
		$data['getkitchen'] =  $this->hungry_model->getkitchen();
		//print_r($data['getkitchen']);die;
/*		print_r($data['special_menu']);die;
*/		if ($this->themeinfo->themename == "modern") {
	...
	public function menu()
	{
		...
		if (empty($this->session->userdata('categoryid'))) {
			...
			$menuid = $this->input->post('category_id');
		} else {
			$categoryid = $this->session->userdata('categoryid');
		}
		//$arrayKey = $this->input->get();
		$getQueryString = $_SERVER["QUERY_STRING"];
		$arrayKey = explode('&', $getQueryString);
		if(count($arrayKey) > 0){
			//$getData = array_values($arrayKey);
			//print_r($arrayKey[0]);die;
			$cateid = $arrayKey[0];
			$categoryid = $arrayKey[1];
		}

		$cateid = $cateid;
		$categoryid = $categoryid;
		$productid = $this->input->post('product_id');
		$sessiondata = array('categoryid' => $categoryid, 'product_id' => $productid,'cateid' => $cateid);
		...
		$data["searchresult"] = $this->hungry_model->searchinfo($product, $category, $config["per_page"], $page,$cateid);
		$data['openclosetime'] =  $this->hungry_model->read_allorderby('*', 'tbl_openclose', 'stid', 'ASC');
