...
class Hungry_model extends CI_Model
{
	...
	public function getcategory()
	{
		...
		$this->db->where('item_category.CategoryIsActive', 1);
		$this->db->limit('3');
		$query = $this->db->get();
		$itemlist = $query->result();
		return $itemlist;
	...
	public function searchinfo($product = null, $category = null, $limit = null, $start = null, $cateid = null)
	{
		$this->db->select('*');
		$this->db->from('item_foods');
		if ((!empty($product)) && (!empty($category))) {
			...
			$this->db->where('item_foods.kitchenid', $cateid);
